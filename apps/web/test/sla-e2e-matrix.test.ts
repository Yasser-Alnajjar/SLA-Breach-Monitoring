...
import type { ZendeskAudit, ZendeskOrganization, ZendeskTicket } from "@sla/zendesk";
...
describe.skipIf(!TEST_DATABASE_URL)("SLA end-to-end test matrix: Zendesk audits -> notification (real Postgres)", () => {
  ...
  beforeEach(async () => {
    ...
    const organization = await prisma.organization.create({ data: { name: "Matrix Org" } });
  ...
  async function ingest(input: { tickets?: ZendeskTicket[]; organizations?: ZendeskOrganization[]; audits?: ZendeskAudit[] }) {
    const inputs = [
  ...
  it("normal -> high: a resolution commitment already 40m into a 2h target survives an upgrade to an 8h target instead of breaching", async () => {
    const normal = await createPolicy("Normal", { priority: ["normal"] }, [{ kind: "resolution", minutes: 120 }]);
    const high = await createPolicy("High", { priority: ["high"] }, [{ kind: "resolution", minutes: 480 }]);
    ...
    expect(row).toMatchObject({ policyVersionId: normal.id, targetMinutes: 120 });

    // 40 minutes in, the customer escalates and the ticket is re-fetched at "high".
    await ingest({ tickets: [ticket(1, { priority: "high", updated_at: iso("10:40") })] });
    ...
    expect(row).toMatchObject({ policyVersionId: high.id, targetMinutes: 480, status: "on_track" });
  ...
  it("high -> normal: a downgrade immediately breaches a commitment whose elapsed time already exceeds the smaller target", async () => {
    ...
    await ingest({ tickets: [t] });
    await runPipelineStages(iso("10:00"));
    expect((await commitmentRow((await caseByExternalId("2")).id, "resolution")).policyVersionId).toBe(high.id);
    ...
    await ingest({ tickets: [ticket(2, { priority: "normal", updated_at: iso("12:10") })] });
    const { reResolved, evaluated } = await runPipelineStages(iso("12:10"));
    expect(reResolved.commitmentsUpdated).toBe(1);

    const row = await commitmentRow((await caseByExternalId("2")).id, "resolution");
    expect(row).toMatchObject({ policyVersionId: normal.id, targetMinutes: 120, status: "breached" });
  ...
  it("normal -> urgent: re-resolves a still-open First Response commitment onto the tighter target, keeping startedAt", async () => {
    const normalPolicy = await createPolicy("Normal", { priority: ["normal"] }, [{ kind: "first_response", minutes: 60 }]);
    const urgent = await createPolicy("Urgent", { priority: ["urgent"] }, [{ kind: "first_response", minutes: 15 }]);
    const t = ticket(3, { priority: "normal" });
    await ingest({ tickets: [t] });
    await runPipelineStages(iso("10:00"));
    const zCase = await caseByExternalId("3");
    expect((await commitmentRow(zCase.id, "first_response")).policyVersionId).toBe(normalPolicy.id);

    await ingest({ tickets: [ticket(3, { priority: "urgent", updated_at: iso("10:05") })] });
    const { reResolved } = await runPipelineStages(iso("10:05"));
    expect(reResolved.commitmentsUpdated).toBe(1);

    const row = await commitmentRow(zCase.id, "first_response");
    expect(row).toMatchObject({ policyVersionId: urgent.id, targetMinutes: 15, startedAt: at("10:00") });
  });
  ...
  it("customer/organization change onto a policy with no target for this commitment kind leaves it unchanged and flags it", async () => {
    const defaultPolicy = await createPolicy("Default", {}, [{ kind: "resolution", minutes: 480 }]);
    ...
    const acmeCustomer = await prisma.customer.findUniqueOrThrow({
      where: { organizationId_zendeskOrgId: { organizationId, zendeskOrgId: "9001" } },
    });
    // Acme's policy only ever configured a First Response target.
    await createPolicy("Acme", { customerIds: [acmeCustomer.id] }, [{ kind: "first_response", minutes: 30 }]);
    ...
    await ingest({ tickets: [ticket(4, { organization_id: 9001, updated_at: iso("10:15") })] });
    ...
    expect(after).toMatchObject({ policyVersionId: defaultPolicy.id, targetMinutes: 480 });
  ...
  describe("finished commitments never change", () => {
    it("a met commitment is untouched by a later priority change", async () => {
      const normalPolicy = await createPolicy("Normal", { priority: ["normal"] }, [{ kind: "first_response", minutes: 60 }]);
      await createPolicy("Urgent", { priority: ["urgent"] }, [{ kind: "first_response", minutes: 15 }]);
      const t = ticket(51, { priority: "normal" });
      await ingest({ tickets: [t], audits: [replyAudit(51, "10:20", "agent")] });
      ...
      await ingest({ tickets: [ticket(51, { priority: "urgent", updated_at: iso("10:30") })] });
      const { reResolved } = await runPipelineStages(iso("10:30"));
      expect(reResolved.commitmentsUpdated).toBe(0);
      const after = await commitmentRow(zCase.id, "first_response");
      expect(after).toMatchObject({ policyVersionId: before.policyVersionId, status: "met", closedAt: before.closedAt });
    });

    it("a closed-and-breached commitment is untouched by a later priority change", async () => {
      const normalPolicy = await createPolicy("Normal", { priority: ["normal"] }, [{ kind: "resolution", minutes: 30 }]);
      ...
      await ingest({ tickets: [t], audits: [statusAudit(52, "11:00", "open", "solved")] });
      await ingest({ tickets: [ticket(52, { priority: "normal", status: "solved", updated_at: iso("11:00") })] });
      await runPipelineStages(iso("11:05"));
      const zCase = await caseByExternalId("52");
      const before = await commitmentRow(zCase.id, "resolution");
      expect(before).toMatchObject({ status: "breached", policyVersionId: normalPolicy.id });
      expect(before.closedAt).not.toBeNull();

      await ingest({ tickets: [ticket(52, { priority: "urgent", status: "solved", updated_at: iso("11:10") })] });
      const { reResolved } = await runPipelineStages(iso("11:10"));
      expect(reResolved.commitmentsUpdated).toBe(0);
      const after = await commitmentRow(zCase.id, "resolution");
      expect(after).toMatchObject({ policyVersionId: before.policyVersionId, status: "breached", closedAt: before.closedAt });
    });

    it("a cancelled commitment is untouched by a later priority change", async () => {
      const normalPolicy = await createPolicy("Normal", { priority: ["normal"] }, [{ kind: "next_reply", minutes: 60 }]);
      await createPolicy("Urgent", { priority: ["urgent"] }, [{ kind: "next_reply", minutes: 15 }]);
      ...
      await ingest({ tickets: [ticket(53, { priority: "urgent", updated_at: iso("10:00") })] });
      ...
      expect(after).toMatchObject({ status: "cancelled", policyVersionId: normalPolicy.id, targetMinutes: 60 });
    });
  });
  ...
  it("a breach is final: a still-open breached commitment stays breached after the matched target increases", async () => {
    ...
    await ingest({ tickets: [ticket(6, { priority: "urgent", updated_at: iso("10:50") })] });
    const { reResolved } = await runPipelineStages(iso("10:50"));
    expect(reResolved.commitmentsUpdated).toBe(0);

    const after = await commitmentRow(zCase.id, "resolution");
    expect(after).toMatchObject({ status: "breached", policyVersionId: before.policyVersionId, targetMinutes: before.targetMinutes });
  ...
  it("D1b: a calendar reassignment alone never re-resolves, but a genuine policy switch carries its calendar into an open cycle and a future one", async () => {
    ...
    const normalPolicy = await createPolicy("Normal", { priority: ["normal"] }, [
    ...
    await ingest({ organizations: [acme] });
    const acmeCustomer = await prisma.customer.findUniqueOrThrow({
      where: { organizationId_zendeskOrgId: { organizationId, zendeskOrgId: "9002" } },
    });

    const t = ticket(7, { priority: "normal", organization_id: 9002 });
    await ingest({
      tickets: [t],
      audits: [replyAudit(7, "10:15", "agent"), replyAudit(7, "10:30", "customer")],
    });
    ...
    expect(cycleBefore).toMatchObject({ status: "on_track", policyVersionId: normalPolicy.id, targetMinutes: 120 });

    // D1b: reassigning the customer's calendar alone must not re-resolve anything.
    await prisma.customer.update({ where: { id: acmeCustomer.id }, data: { calendarId: narrowCalendar.id } });
    const calendarOnly = await runPipelineStages(iso("10:35"));
    expect(calendarOnly.reResolved.commitmentsUpdated).toBe(0);
    expect((await commitmentRow(zCase.id, "resolution")).calendarVersionId).toBe(calendar247Id);
    expect((await commitmentRow(zCase.id, "next_reply")).calendarVersionId).toBe(calendar247Id);
    ...
    await ingest({ tickets: [ticket(7, { priority: "urgent", organization_id: 9002, updated_at: iso("10:40") })] });
    ...
    expect(anchorAfter).toMatchObject({ id: anchorBefore.id, policyVersionId: urgent.id, targetMinutes: 60, calendarVersionId: narrowCalendarVersionId });
    ...
    await ingest({ tickets: [ticket(7, { priority: "urgent", organization_id: 9002, updated_at: iso("10:45") })], audits: [replyAudit(7, "10:45", "agent")] });
    await runPipelineStages(iso("10:45"));
    await ingest({ tickets: [ticket(7, { priority: "urgent", organization_id: 9002, updated_at: iso("11:00") })], audits: [replyAudit(7, "11:00", "customer")] });
    await runPipelineStages(iso("11:00"));

    const cycleRows = await prisma.commitment.findMany({ where: { caseId: zCase.id, kind: "next_reply" }, orderBy: { startedAt: "asc" } });
    expect(cycleRows).toHaveLength(2);
    expect(cycleRows[1]).toMatchObject({ startedAt: at("11:00"), policyVersionId: urgent.id, targetMinutes: 45, calendarVersionId: narrowCalendarVersionId });
  });
  ...
  it("a breach reached through re-resolution reaches the notification stage and sends exactly one Slack alert", async () => {
    await prisma.slackIntegration.create({
      data: { organizationId, accessToken: "xoxb-test", teamId: "T123", teamName: "Matrix", botUserId: "U123", channelId: "C123" },
    });

    const normalPolicy = await createPolicy("Normal", { priority: ["normal"] }, [{ kind: "resolution", minutes: 480 }]);
    await createPolicy("Urgent", { priority: ["urgent"] }, [{ kind: "resolution", minutes: 30 }]);
    const t = ticket(8, { priority: "normal" });
    await ingest({ tickets: [t] });
    await runPipelineStages(iso("10:00"));
    expect((await commitmentRow((await caseByExternalId("8")).id, "resolution")).policyVersionId).toBe(normalPolicy.id);

    // Escalating to Urgent's 30-minute target after 45 elapsed minutes breaches outright.
    await ingest({ tickets: [ticket(8, { priority: "urgent", updated_at: iso("10:45") })] });
    ...
    expect(evaluated.notificationCandidates).toEqual([expect.objectContaining({ commitmentId: row.id, threshold: 100, status: "breached" })]);
    expect(sent.notificationsSent).toBe(1);
    expect(postMessage).toHaveBeenCalledTimes(1);

    const notificationRow = await prisma.notification.findFirstOrThrow({ where: { commitmentId: row.id } });
