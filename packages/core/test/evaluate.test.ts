...
import type { BusinessCalendarVersion, Commitment, NormalizedEvent, SLAPolicyVersion } from "../src/types.js";
...
const baseEvents: NormalizedEvent[] = [
  { id: "evt-1", caseId: "case-1", type: "case_created", occurredAt: "2026-09-07T09:00:00.000Z", actor: "customer", system: "zendesk", fromState: null, toState: "open", sourceRawEventId: "raw-1" },
];

function minutesAfterStart(minutes: number): string {
  return new Date(new Date(commitment.startedAt).getTime() + minutes * 60_000).toISOString();
}

describe("evaluateCommitment", () => {
  it("is on_track below the first warning threshold", () => {
    const evaluation = evaluateCommitment(commitment, baseEvents, policy, alwaysOpen, minutesAfterStart(100));
  ...
  it("transitions to at_risk once a warnAtPercent threshold is crossed", () => {
    ...
    const evaluation = evaluateCommitment(commitment, baseEvents, policy, alwaysOpen, minutesAfterStart(200));
  ...
  it("transitions to breached once elapsed exceeds the target", () => {
    const evaluation = evaluateCommitment(commitment, baseEvents, policy, alwaysOpen, minutesAfterStart(250));
  ...
  it("resolves to met when closed inside the target", () => {
    const events = [...baseEvents, { id: "evt-close", caseId: "case-1", type: "case_closed" as const, occurredAt: minutesAfterStart(50), actor: "agent" as const, system: "zendesk" as const, fromState: "open" as const, toState: "resolved" as const, sourceRawEventId: "raw-close" }];
    const evaluation = evaluateCommitment(commitment, events, policy, alwaysOpen, minutesAfterStart(300));
  ...
  it("resolves to breached when closed past the target", () => {
    const events = [...baseEvents, { id: "evt-close", caseId: "case-1", type: "case_closed" as const, occurredAt: minutesAfterStart(260), actor: "agent" as const, system: "zendesk" as const, fromState: "open" as const, toState: "resolved" as const, sourceRawEventId: "raw-close" }];
    const evaluation = evaluateCommitment(commitment, events, policy, alwaysOpen, minutesAfterStart(300));
    expect(evaluation.status).toBe("breached");
  ...
  it("is reproducible: identical inputs called twice produce an identical Evaluation, id included", () => {
    ...
    const first = evaluateCommitment(commitment, baseEvents, policy, alwaysOpen, asOf);
    const second = evaluateCommitment(commitment, baseEvents, policy, alwaysOpen, asOf);
