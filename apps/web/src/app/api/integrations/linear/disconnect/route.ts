...
/**
 * Soft-disconnects the Linear integration: clears credentials and marks the
 ...
 *
 * No vendor-side token revocation call is made: Linear's OAuth API documents
 * no self-service revoke endpoint, so only the local credentials are cleared.
 */
export async function POST() {
  ...
  const integration = await prisma.integration.findUnique({
    where: { organizationId_provider: { organizationId: session.user.organizationId, provider: "linear" } },
  });
