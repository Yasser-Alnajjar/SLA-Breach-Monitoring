...
async function readCredentials(prisma: PrismaClient, integrationId: string): Promise<LinearCredentials> {
  const integration = await prisma.integration.findUniqueOrThrow({ where: { id: integrationId } });
  return integration.credentials as unknown as LinearCredentials;
}
...
export async function loadFreshLinearCredentials(
  ...
  if (current.reauthRequired) throw new LinearReauthRequiredError();
  return current;
}
