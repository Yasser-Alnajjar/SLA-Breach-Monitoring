...
describe("extractMatchFromFilter", () => {
  it("preserves a single priority condition as a generic condition", () => {
  ...
  it("preserves all + any priority conditions without flattening their logical structure", () => {
    const { match, unsupportedConditions } = extractMatchFromFilter(
      {
        all: [
          {
  ...
  it("translates an organization_id condition to our internal Customer id and drops the redundant generic condition", () => {
    ...
    // organization_id is fully represented by `customerIds` — left in the
    // generic `conditions` group too, it would always fail to match (a case's
    // attributes only ever carry `customerId`, never `organization_id`) and
    // silently defeat the working customerIds check.
    expect(match).toEqual({
      customerIds: ["cust_abc"],
      conditions: {},
    });
  ...
  it("drops all organization_id conditions from the generic group while retaining resolved legacy Customer ids", () => {
    const customerIdsByZendeskOrgId = new Map([["7", "cust_abc"]]);
  ...
  it("drops organization_id but keeps sibling conditions in the same group", () => {
    const customerIdsByZendeskOrgId = new Map([["7", "cust_abc"]]);
    ...
    const { match } = extractMatchFromFilter(
      {
        all: [
          ...
          { field: "tags", operator: "contains", value: "d6" },
        ],
      },
      customerIdsByZendeskOrgId,
    );
  ...
  it("preserves arbitrary Zendesk condition fields and operators", () => {
    const { match, unsupportedConditions } = extractMatchFromFilter(
      {
        all: [
          {
  ...
  it("returns an empty match (matches everything) for an undefined filter", () => {
    expect(extractMatchFromFilter(undefined, new Map())).toEqual({
      match: {},
...
describe("groupPolicyMetricsByPriority", () => {
  function metric(
    overrides: Partial<ZendeskSlaPolicyMetric>,
  ...
  it("splits metrics into separate groups per priority tier", () => {
    ...
    expect(
      groups.find((group) => group.priority === "urgent")?.targets,
    ).toEqual([
      {
        kind: "first_response",
        minutes: 30,
    ...
    expect(groups.find((group) => group.priority === "low")?.targets).toEqual([
      {
        kind: "first_response",
        minutes: 240,
      },
  ...
  it("returns no groups for an undefined metrics array", () => {
    expect(groupPolicyMetricsByPriority(undefined)).toEqual({
      groups: [],
...
describe("policyVersionContentEquals", () => {
  ...
  it("is false when a target's minutes changed", () => {
    expect(
      policyVersionContentEquals(base, {
        ...base,
        targets: [
          {
            kind: "first_response",
...
describe("resolvePolicyCalendarVersion", () => {
  ...
  it("uses the default calendar when the policy has no schedule_id", () => {
    const result = resolvePolicyCalendarVersion(
      policy({
        schedule_id: null,
      }),
      new Map(),
