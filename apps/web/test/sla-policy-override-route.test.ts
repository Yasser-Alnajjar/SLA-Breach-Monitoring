...
describe.skipIf(!TEST_DATABASE_URL)(
  "SLA policy override route (real Postgres)",
  () => {
    ...
    it("rejects an unknown commitment kind and writes nothing", async () => {
      const response = await route.POST(
        jsonRequest(url, {
          policyId,
          targets: [{ kind: "bogus_kind", minutes: 30 }],
        }),
      );

      expect(response.status).toBe(400);
      expect(await prisma.sLAPolicyVersion.count({ where: { policyId } })).toBe(
        1,
      );
    });
  },
