...
|                          |                                                                                                                                                                                                                                                                                                                                          |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Now**                  | Phase 5 — Team & Account Management, 3/9 tasks done (2026-09-22): D8 decided yes, 5.1 (deployment SMTP), 5.2 (invitations) and 5.3 (members page) code-complete. Phase 3 — Explainable Cases 10/10 tasks code-complete, merged to `main` (PR #24); live-account verification (assignee resolution, priority-change events) is still owner-only pending there |
| **Up next**              | 5.4 Authorization audit of every mutation — owner vs. member, per-route                                                                                                                                                                                                                                                                    |
...
**Status:** 🔄 In progress (3/9 tasks done, 2026-09-22) · branch `phase/5-team-account-management` · **Estimate:** 2 weeks · **Needs:** D8 — decided
...
- [x] **5.3** Members page: list members, change roles, remove members. `P1 · Feature · C` · S-2 (done 2026-09-22)
  - Status (2026-09-22): traced `authz.ts` first (per this task's own guardrails, and repeating 5.2's own trace) — it only gates the one deployment-level "platform operator" action (Worker/Monitoring settings) and has no `UserRole` concept at all; every other settings mutation, invitations included, is documented as intentionally open to any signed-in member. Kept that convention: no `requireOwner()`-style gate was added for role-change/remove — role restriction stays 5.4's job, as 5.2 already established for invitations. What this task *did* add, as a correctness invariant rather than authorization: an organization may never end up with zero owners (nothing enforced this anywhere before — grepped the whole app for owner-counting logic and found none), and a signed-in user may never remove their own account through this page (there's no `User.sessionVersion`/server session store yet — roadmap 5.7 — so a self-removal would leave the caller mid-request holding a session for a `User` row that no longer exists).
  - New `packages/db/src/members.ts`: `listMembers` (org-scoped, oldest-first), `updateMemberRole` (idempotent no-op if the role is unchanged; blocks demoting an organization's only `owner`), `removeMember` (blocks removing yourself outright, and blocks removing an organization's only `owner`) — three new error classes (`MemberNotFoundError`, `CannotRemoveSelfError`, `LastOwnerError`). Mutations are scoped `updateMany`/`deleteMany` by `(id, organizationId)`, matching `revokeInvitation`'s idempotent-and-scoped shape (5.2). No cascading-delete work was needed: `OrganizationInvitation.invitedByUserId`/`acceptedByUserId` are already `SetNull` on delete — added by 5.2 specifically so this task's member removal would never be blocked by a user's invitation history — and no other model in the schema references `User` at all.
  - New routes: `GET /api/settings/members` (list) and `PATCH`/`DELETE /api/settings/members/[id]` (role change / remove), session-gated exactly like every other settings route (`proxy.ts` needed no changes — `/api/settings/**` is already caught by the generic authenticated-route check). Typed errors map to `404` (not found), `409` (last-owner invariant), `400` (self-removal) — same `instanceof`-dispatch convention as the invitations routes.
  - Extended the existing `apps/web/modules/settings/members` module (5.2's invite-only page) rather than adding a new page: `MembersView.tsx` gained a "Members" card above the invite form, one row per member (role `Select` + `Remove` button, following `CustomerCalendarsCard.tsx`'s per-row local-loading-state pattern), with the current user's own row's Remove button disabled client-side (the server would reject it anyway, but this avoids a confusing round-trip). `ssr/Members.tsx` now fetches invitations and members in parallel (`Promise.all`) via a new `Actions.Members.getData()` (returns the list plus `session.user.id`, needed to render the "(you)" row and disable self-removal) and a new `Actions.Members.updateRole`/`remove` in `@/actions/client`. Mutations refresh the page the same way invitations already do — `router.refresh()`, no client-side cache.
  - Tests: `packages/db/test/members.test.ts` (11 cases, in-memory fake `user` delegate — org-scoping, the no-op-if-unchanged path, both last-owner guards, and that removing one of two owners is allowed) and `apps/web/test/member-settings-routes.test.ts` (12 cases, Pattern A mocked with real error classes via `importActual`, same as `invitation-settings-routes.test.ts`) — 401/400/404/409 status mapping and organization-scoping for every route.
  - Verified: `apps/web` `tsc --noEmit` clean (`packages/db` reproduces the same pre-existing rootDir `tsc` failure on files this task never touched, noted throughout Phase 5); full non-DB unit suite 118 files / 1446 tests passing (up from 116/1423 at 5.2, i.e. exactly this task's 23 new tests, zero regressions — confirmed by running the unchanged suite on a stashed tree first). **UI checked in a real browser** — unlike Phases 2–5's usual "no dev environment" gap, a live Postgres was actually running this session: signed up a disposable test organization (never touched the pre-existing accounts in that database) and drove `/settings/members` end to end — the sole owner's own Remove button is disabled with the "You can't remove yourself" tooltip; demoting the sole owner to `member` is rejected server-side (`409`, "An organization must have at least one owner") and the row visibly reverts; after seeding a second `member` directly (no SMTP configured to actually deliver an invite email in this environment), promoting them to `owner` and then removing them both succeeded, with the member-count copy ("1 person has"/"2 people have") updating correctly. The disposable org and its two users were deleted afterward.
