...
|                          |                                                                                                                                                                                                                                                  |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Now**                  | Phase 5 — Team & Account Management, 1/9 tasks done (2026-09-22): D8 decided yes, 5.1 (deployment-level transactional email) code-complete. Phase 3 — Explainable Cases 10/10 tasks code-complete, merged to `main` (PR #24); live-account verification (assignee resolution, priority-change events) is still owner-only pending there |
| **Up next**              | 5.2 Invitations (needs 5.1's transactional mailer, now available)                                                                                                                                                                                 |
...
| [5](#phase-5--team--account-management)                | Team and account management                                                           | 2 wk | 🔄 In progress (1/9, 2026-09-22)                                             |
...
- [x] **D8** — Deployment-level transactional email (`TRANSACTIONAL_SMTP_*`) for invites, password resets and verification? **Decided: yes** (2026-09-22), after an audit of the existing SMTP implementation. `OrganizationEmailSettings` is per-org, opt-in, and never auto-provisioned at signup — so it has no config to send through for exactly the flows D8 covers: the first invite, a password reset for an owner who never opened Settings → Notifications, or verification at signup itself. Reusing it would make core account-recovery/access flows depend on an unrelated, optional config. `TRANSACTIONAL_SMTP_*` follows the same deployment-level, purpose-scoped pattern the codebase already uses for `OPS_ALERT_SMTP_*`, and reuses `packages/email`'s existing `sendEmail`/`EmailConfig` transport unchanged — only a new config source, no new mail-sending code. → unblocks 5.1
...
**Status:** 🔄 In progress (1/9 tasks done, 2026-09-22) · branch `phase/5-team-account-management` · **Estimate:** 2 weeks · **Needs:** D8 — decided
