...
|                          |                                                                                                                                                                                                                                                  |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Now**                  | Phase 4 — SLA Policy & Calendar Management, 8/8 tasks code-complete (2026-09-22); live verification (owner-only, mirrors 2.9) remains before the phase can close. Phase 3 — Explainable Cases 10/10 tasks code-complete, merged to `main` (PR #24); live-account verification also still pending there |
| **Up next**              | Phase 5 — Team & Account Management                                                                                                                                                                                                              |
| **Blocked on decisions** | D8 (Phase 5) · D11 (scheduling)                                                                                                                                                                                                                  |
...
| [4](#phase-4--sla-policy--calendar-management)         | SLA policy and calendar management inside Watchtower                                  | 4 wk | 🔄 In progress (8/8 code-complete, 2026-09-22) — live verification pending  |
...
**Status:** 🔄 In progress (8/8 tasks code-complete, 2026-09-22) — not yet ✅: needs a live-verification pass (migrations against a real Postgres, the 4 new real-Postgres test suites, and the UI in a browser — no live dev environment was available in this session, per Phase 2's 2.9 and Phase 3's precedent) before closing the phase and opening its PR. · **Estimate:** 4 weeks · **Needs:** D1, D1b, D12 — all three decided
...
**Phase 4 summary (2026-09-22):** all 8 tasks code-complete. `packages/core`/`packages/db`/`packages/commitments`/`packages/zendesk`/`apps/web`/`apps/worker`/`apps/concierge` all `tsc --noEmit` clean; full non-DB unit suite passing (1263/1263, including 21 new tests). **Not verified**: the new migrations against a real Postgres, the 4 new real-Postgres test suites, and the UI in an actual browser — no Docker/Postgres or a live dev environment was available in this session, mirroring the exact limitation Phases 2 and 3 both hit and documented the same way. Both "Phase is done when" conditions are implemented and covered by tests that need that live verification to actually run: an org with no Zendesk policies gets commitments from its own native policies (D12 precedence + native policy/calendar creation), and a local edit to an imported calendar survives the next Zendesk sync (E-18 fix + regression test). Recommend the same owner-only live-verification pass Phase 2's 2.9 and Phase 3's pending items describe before opening the Phase 4 → main PR.
