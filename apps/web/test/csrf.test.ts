...
const { isSameOriginRequest, isStateChangingMethod } = await import("../src/lib/csrf");
...
describe("isStateChangingMethod", () => {
  it("treats GET/HEAD/OPTIONS as safe and everything else as state-changing", () => {
    for (const m of ["GET", "HEAD", "OPTIONS", "get"]) expect(isStateChangingMethod(m)).toBe(false);
...
describe("isSameOriginRequest", () => {
  it("accepts an Origin matching NEXTAUTH_URL", () => {
    expect(isSameOriginRequest(request("/x", { headers: { origin: APP_URL } }), APP_URL)).toBe(true);
  });

  it("rejects a foreign Origin even when Host is ours", () => {
    const req = request("/x", { headers: { origin: "https://evil.example", host: "app.example.com" } });
    expect(isSameOriginRequest(req, APP_URL)).toBe(false);
  });

  it("rejects a sibling subdomain (same-site but cross-origin)", () => {
    const req = request("/x", { headers: { origin: "https://evil.example.com" } });
    expect(isSameOriginRequest(req, APP_URL)).toBe(false);
  });

  it("rejects a different scheme or port on the same host", () => {
    expect(isSameOriginRequest(request("/x", { headers: { origin: "http://app.example.com" } }), APP_URL)).toBe(false);
    expect(isSameOriginRequest(request("/x", { headers: { origin: "https://app.example.com:8443" } }), APP_URL)).toBe(false);
  });
  ...
  it("rejects a missing Origin with no Referer, and a literal 'null' Origin", () => {
    ...
    expect(isSameOriginRequest(request("/x", { headers: { origin: "null" } }), APP_URL)).toBe(false);
  });

  it("does not fall back to Referer when an Origin header is present but unusable", () => {
    const req = request("/x", { headers: { origin: "null", referer: `${APP_URL}/settings` } });
    expect(isSameOriginRequest(req, APP_URL)).toBe(false);
  });

  it("falls back to Referer's origin when Origin is absent", () => {
    expect(isSameOriginRequest(request("/x", { headers: { referer: `${APP_URL}/settings` } }), APP_URL)).toBe(true);
    expect(isSameOriginRequest(request("/x", { headers: { referer: "https://evil.example/page" } }), APP_URL)).toBe(false);
  });

  it("accepts an Origin matching the addressed host (dev over a LAN IP / tunnel)", () => {
    const req = request("/x", { headers: { origin: "http://192.168.1.46:3000", host: "192.168.1.46:3000" } });
  ...
  it("prefers X-Forwarded-Host over Host behind the reverse proxy", () => {
    const req = request("/x", {
      headers: { origin: "https://sla.customer.io", host: "web:3000", "x-forwarded-host": "sla.customer.io" },
    });
...
describe("proxy Origin check", () => {
  ...
  it.each([
    ...
    const response = await proxy(request(path, { method, headers: { origin: "https://evil.example" } }));
    expect(response.status).toBe(403);
  });

  it("lets a same-origin state-changing request through to the auth check", async () => {
    const response = await proxy(request("/api/settings/email", { headers: { origin: APP_URL } }));
  ...
  it("does not require Origin on GET requests", async () => {
    const response = await proxy(request("/api/settings/email", { method: "GET" }));
    expect(response.status).toBe(401);
  });
