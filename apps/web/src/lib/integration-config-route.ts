 ...
 */
export function createIntegrationConfigHandlers(provider: ConfigurableIntegrationProvider) {
  async function GET() {
    const session = await getServerSession(authOptions);
    if (!session) return NextResponse.json({ error: "Not signed in" }, { status: 401 });

    const status = await getIntegrationConfigStatus(getPrismaClient(), session.user.organizationId, provider);
  ...
  async function POST(request: Request) {
    ...
    try {
      await saveIntegrationConfig(getPrismaClient(), session.user.organizationId, provider, {
        clientId,
