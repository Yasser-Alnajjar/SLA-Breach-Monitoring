...
- **Read-only access, with one documented exception:** Zendesk, Jira, Linear, and Intercom are connected under OAuth scopes that grant read-only access. GitHub is the one exception, and it's a limitation of GitHub's own OAuth model, not a choice made by this product: GitHub's classic OAuth apps have no scope that grants read access to a private repository's pull requests without also granting write access to that repository (the `repo` scope). The product requests that scope because it's the narrowest one GitHub offers for this purpose, but **it never uses it to write** — no pull request, comment, status, or file is ever created or modified through this connection.
...
Yes, for every connected data source (Zendesk, Jira, Linear, Intercom, GitHub) — it only reads. The only outbound writes anywhere in the product are a Slack message and an alert email, and GitHub's OAuth scope is broader than the others purely because GitHub itself has no narrower read-only scope for private-repo pull requests (Section 22) — the product never uses that scope to write.
